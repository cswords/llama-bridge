...
| 模型配置 | 规模 | 可用性 | 启动耗时 | 响应特征 |
| :--- | :--- | :--- | :--- | :--- |
| **Qwen 2.5 3B** | 3B | ✅ Ready | 极快 (<2s) | 即时响应，无感知延迟 |
| **Qwen 30B Coder** | 30B | ✅ Ready | 中等 (~20s) | 首字响应迅速 |
| **MiMo V2 (Sonnet)** | ~70B | ✅ Ready | 较慢 (~40s+) | **冷热不均**：首个请求包含 Metal 编译开销 (~0.7s)，后续请求显著加速 (~0.4s) |
| **GLM-4 (REAP)** | 218B | ✅ Ready | 慢 (~50s+) | 性能受限于内存带宽，冷热请求差异不明显 |

> **注意**:
> 1. **大模型启动**: 70B+ 模型在初次加载时可能看起来像“卡住”了一样（等待时间视磁盘 IO 而定，可能长达 1-2 分钟），这是正常的。请耐心等待直到端口 (8000) 绑定。
> 2. **首请求延迟**: 服务启动后的第一个请求通常会触发 Metal Shader 的编译缓存，比后续请求稍慢。
> 3. **内存需求**: `mimo-v2.toml` 默认配置了 256K Context，需要极高的系统内存。如遇 OOM 或系统卡顿，请在配置文件中降低 `n_ctx`。
